...
void createInstance() {
    ...
    appInfo.pEngineName = "blahajEngine";
...
void createSurface() {
    if (glfwCreateWindowSurface(instance, vulkanWindow, nullptr, &surface) != VK_SUCCESS) {
...
void blahajEngine::VulkanRenderer::init(GLFWwindow* windowi) {
    vulkanWindow = windowi;
...
void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, std::vector<std::shared_ptr<gameObject>> gameObjects, std::vector<std::shared_ptr<gameObject>> gameObjectsStaging) {
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
void blahajEngine::VulkanRenderer::drawFrame(std::vector<std::shared_ptr<blahajEngine::gameObject>>& gameObjects, std::vector<std::shared_ptr<blahajEngine::gameObject>>& gameObjectsStaging) {
    ...
    recordCommandBuffer(commandBuffers[currentFrame], imageIndex, gameObjects, gameObjectsStaging);
...
void blahajEngine::VulkanRenderer::createTextureImage(int id, std::string imagePath) {
...
void blahajEngine::VulkanRenderer::createDescriptorSets(int id) {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        this->createTextureImage(1, "assets/null.png");
...
void blahajEngine::VulkanRenderer::createGraphicsPipeline(int id, std::string vertShaderPath, std::string fragShaderPath) {
...
void blahajEngine::VulkanRenderer::createVertexBuffer(int id, std::vector<Vertex> vertices) {
...
void blahajEngine::VulkanRenderer::createIndexBuffer(int id, std::vector<uint16_t> indices) {
...
void blahajEngine::VulkanRenderer::cleanup(std::vector<std::shared_ptr<blahajEngine::gameObject>>& gameObjects) {
    cleanupSwapChain();
...
VkExtent2D blahajEngine::VulkanRenderer::getSwapChainExtent() const {
    return swapChainExtent;
}
void blahajEngine::VulkanRenderer::setSwapChainExtent(const VkExtent2D& extent) {
    swapChainExtent = extent;
}

const camera& blahajEngine::VulkanRenderer::getCamera() const {
    return cam;
}
void blahajEngine::VulkanRenderer::setCamera(const camera& c) {
    cam = c;
}

const std::map<int, std::vector<Vertex>>& blahajEngine::VulkanRenderer::getVertices() const {
    return vertices;
}
void blahajEngine::VulkanRenderer::setVertices(const std::map<int, std::vector<Vertex>>& v) {
    vertices = v;
}

const std::map<int, std::vector<uint16_t>>& blahajEngine::VulkanRenderer::getIndices() const {
    return indices;
}
void blahajEngine::VulkanRenderer::setIndices(const std::map<int, std::vector<uint16_t>>& i) {
    indices = i;
}

bool blahajEngine::VulkanRenderer::isFramebufferResized() const {
    return framebufferResized;
}
void blahajEngine::VulkanRenderer::setFramebufferResized(bool resized) {
    framebufferResized = resized;
}

GLFWwindow* blahajEngine::VulkanRenderer::getWindow() const {
    return vulkanWindow;
}
void blahajEngine::VulkanRenderer::setWindow(GLFWwindow* win) {
    vulkanWindow = win;
}

blahajEngine::VulkanRenderer::VulkanRenderer() {
