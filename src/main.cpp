...
int main() {
    ...
    app.setGameLoopCallback([&snake, &directionX, &directionY, &foodX, &foodY](blahajEngine& instance) {
        // Handle input
        if (glfwGetKey(instance.window, GLFW_KEY_W) == GLFW_PRESS && directionX == 0) {
            directionX = -1; directionY = 0;
        }
        if (glfwGetKey(instance.window, GLFW_KEY_A) == GLFW_PRESS && directionY == 0) {
            directionX = 0; directionY = -1;
        }
        if (glfwGetKey(instance.window, GLFW_KEY_S) == GLFW_PRESS && directionX == 0) {
            directionX = 1; directionY = 0;
        }
        if (glfwGetKey(instance.window, GLFW_KEY_D) == GLFW_PRESS && directionY == 0) {
            directionX = 0; directionY = 1;
        }
        ...
        for (auto& segment : snake) {
            tileMap[segment.y][segment.x] = 1;
        }
        tileMap[foodY][foodX] = 2; // Food
        ...
        for (int y = 0; y < instance.bgTileMapHeight; y++) {
            for (int x = 0; x < instance.bgTileMapWidth; x++) {
                if (index < instance.bgTileMapArrayLength) {
                    instance.bgTileMap[index++] = tileMap[y][x];
                }
            }
        }

        // Reverse array for as needed for blahajEngine
        reverseArray(instance.bgTileMap, instance.bgTileMapArrayLength);
    });
